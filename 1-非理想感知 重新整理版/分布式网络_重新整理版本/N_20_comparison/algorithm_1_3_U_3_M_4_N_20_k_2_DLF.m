...
% 需要注意 P_d 为 0.8、0.9， P_fa 取值为 0.3、0.2，因为在后续 Reward_expect_all 中仅考虑这几种情况
P_d = 0.8;  % 检测概率
...
%     estimated_prob_user_3 = channel_free_prob;
    ...
    for t = 2:sample_num
        for i = 1:N
            ...
            indexes_user_1_(i) = -1 * (estimated_prob_user_1(i) - sqrt( 2*log(t-1)/T_user_1(i) ) /(P_d-P_fa));
        end
        ...
        for i = 1:N
            ...
            indexes_user_2_(i) = -1 * (estimated_prob_user_2(i) - sqrt( 2*log(t-1)/T_user_2(i) ) /(P_d-P_fa));
        end
        ...
        for i = 1:N
            ...
            indexes_user_3_(i) = -1 * (estimated_prob_user_3(i) - sqrt( 2*log(t-1)/T_user_3(i) ) /(P_d-P_fa));
        end     
...
%         u_3 = 3;
        ...
        % 注意的是 sequence_1_ 用来存储 SL(K) 算法中式 (5)
        % 的顺序，原式取最小值，本仿真中取负号取最大值，为了与后续逻辑对应
        [value_1 sequence_1_] = sort(indexes_user_1_( sequence_1( 1:U*M ) ), 'descend' );
        [value_2 sequence_2_] = sort(indexes_user_2_( sequence_2( 1:U*M ) ), 'descend' );
        [value_3 sequence_3_] = sort(indexes_user_3_( sequence_3( 1:U*M ) ), 'descend' );
        
        M_sensed_channels_1 = sequence_1_( (u_1-1)*M+1: u_1 *M  );
        M_sensed_channels_2 = sequence_2_( (u_2-1)*M+1: u_2 *M   );
        M_sensed_channels_3 = sequence_3_( (u_3-1)*M+1: u_3 *M   );
...
%         channel_chosen_access_3 = zeros(1,K);
        ...
        % 为了对应 算法 SL(K) 中式子(5) 中最小的信道，因此将该值取负数求最大的信道 
        for i = 1:M
            temp_indexes_1(i) = -1 * ((Y_user_1(M_sensed_channels_1(i))/ T_user_1(M_sensed_channels_1(i))+ P_d-1)/(P_d - P_fa) - sqrt(2*log(t-1)/T_user_1(M_sensed_channels_1(i)) )/(P_d-P_fa) );
        end  
        for i = 1:M
            temp_indexes_2(i) = -1 * ((Y_user_2(M_sensed_channels_2(i))/ T_user_2(M_sensed_channels_2(i))+ P_d-1)/(P_d - P_fa) - sqrt(2*log(t-1)/T_user_2(M_sensed_channels_2(i)) )/(P_d-P_fa));
        end  
        for i = 1:M
            temp_indexes_3(i) = -1 * ((Y_user_3(M_sensed_channels_3(i))/ T_user_3(M_sensed_channels_3(i))+ P_d-1)/(P_d - P_fa) - sqrt(2*log(t-1)/T_user_3(M_sensed_channels_3(i)) )/(P_d-P_fa));
        end         
...
%% benchmark 该值由 obtain_benchmark 求得后添加至此
if P_d == 0.8 && P_fa == 0.3
    Reward_expect_all = 1.2963 + 1.5214 + 1.6453 ; 
elseif P_d == 0.9 && P_fa == 0.3
    Reward_expect_all = 1.6709 + 1.5484 + 1.3190 ; 
elseif P_d == 0.8 && P_fa == 0.2
    Reward_expect_all = 1.4369 + 1.6479 + 1.7568;  
elseif P_d == 0.9 && P_fa == 0.2
    Reward_expect_all = 1.7864 + 1.6796 + 1.4643;     
else
    print("参数选取错误，P_d 和 P_fa 需满足某些条件")
end
...
% save 'N_20_U_3_M_4_K_2_regret_DLF.mat' save_result2
% save 'N_20_U_3_M_4_K_2_reward_DLF.mat' save_result1
% save 'N_20_U_3_M_4_K_2_reward_optimal.mat' Reward_expect_all
