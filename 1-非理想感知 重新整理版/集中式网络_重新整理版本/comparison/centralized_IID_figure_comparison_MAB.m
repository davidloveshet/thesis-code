...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Description : This is used to simulate the centralized algorithm in Chap
% 3.2: Figure 3.9 and Figure 3.10
%               
%               Author: xxx
%               Date  : 2018/03/22
%               Modified : 2023/05/01
...
% 注意此处考虑经典 MAB 算法，检测概率、虚警概率用于信道状态，在后续过程中设置了 P_d_temp = 1 和 P_fa_temp = 0
% 用于计算信道索引标号
...
for n = 1:Montecalo_num
    ...
    for t = 2:sample_num
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %% 此处为了后续算法中不考虑检测概率、虚警概率的影响，使得后续算法为经典 MAB 算法
        P_d_temp = 1;
        P_fa_temp = 0;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       for i = 1:N
           estimated_prob(i) = (Y(i)/T(i) + (P_d_temp-1))/(P_d_temp-P_fa_temp);
           indexes(i) = estimated_prob(i) + sqrt( 2*log(t-1)/T(i) ) /(P_d_temp-P_fa_temp); 
       end
       ...
       for i = 1:M
           temp_indexes(i) = (Y(M_sensed_channels(i))/T(M_sensed_channels(i)) + P_d_temp-1)/(P_d_temp - P_fa_temp) + sqrt(2*log(t-1)/T(M_sensed_channels(i)) )/(P_d_temp-P_fa_temp);
...
hold off;
% 
% save 'P_d_MAB_regret.mat' Regret_as_time
% save 'P_d_MAB_reward.mat' Reward_average_as_time
% save 'P_d_MAB_reward_optimal.mat' Reward_expect_all_status
