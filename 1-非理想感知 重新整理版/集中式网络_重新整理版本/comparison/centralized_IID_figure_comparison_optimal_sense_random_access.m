...
for n = 1:Montecalo_num
    ...
    for t = 2:sample_num
       ...
       %% 感知前 M 条信道，更新 T 和 Y
       % 将 感知到的 M 条信道打乱，表示感知到 M 条信道，但是在后续接入中随机接入
       M_sensed_channels = M_sensed_channels([randperm(length(M_sensed_channels))]);
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       for i = 1:M
...
% save 'P_d_sense_optimal_random_access_regret.mat' Regret_as_time
% save 'P_d_sense_optimal_random_access_reward.mat' Reward_average_as_time
% save 'P_d_sense_optimal_random_access_reward_optimal.mat' Reward_expect_all_status
