...
- 调整 U 的数量，分别对应不同的用户数量；
- 该代码仿真时间会有好几个小时 
- 时间仓促未来得及完全注释，如有问题请随时联系: sssss976@163.com ，将全面细致解答
...
# In[0] 定义拍卖算法，实际上我们在分布式中用到的该算法，在本代码中并未使用该部分。但是可以用来检验信道分配是否准确。
...
for index_, budget in enumerate(budget_list):
    ...
    simulation_Num_ = math.ceil(budget * 3000) # 仿真预留的时隙数量要高于能量耗尽前的时隙数量
