...
This file is used for comparison of multiple algos

下面算法对 363-374行 进行不同注释可得到不同算法的仿真结果
...
transmission_rate = 1 # M bits per second
duration = 95e-03 # s
bandwidth = 1 # MHz
...
class algo:
    ...
    # random algorithm
    def compute_index_update_random(self, time):
        self.index  = np.random.random()
        return self.index

    # c-UCB algorithm
    def compute_index_update_c(self, time):
    ...
    def compute_index_update_ONES(self, time, g_star, channel_Num):
...
for u in range(U):
    ...
    for i in range(N):
        channel_SU_set[u].append( channel_info( Tx_antenna_Num, c_sensing, transmission_rate, duration, bandwidth, transmission_distance[u], Channel_Noise_Set[u][i]   ) )
        ...
        for num in range(simulation_Num_):
            ...
            for u in range(U_temp):
                for i in range(N):
                    ...
                    Power_Requirement_temp, Energy_Requirement_temp = channel_SU_set[u][i].Energy_minimum_() # 求得所需能量大小
            ...
            if num < 1:
                ...
                for u in range(U):
                    for i in range(N):
                        ...
                        # random algorithm
                        # index_U_N[u][i] = policy_SU_set[u][i].compute_index_update_random( num )

                        # c-UCB algorithm
                        # index_U_N[u][i] = policy_SU_set[u][i].compute_index_update_c( num )
                ...
                # for u in range(U):        
                #     for i in range(N):                  
                #         index_U_N[u][i] = policy_SU_set[u][i].compute_index_update_ONES(num, g_star[u], N)    
                ...
                # """
                # hungarian 算法模块 in unknown environment
                hungarian = Hungarian()
