...
class algo:
    ...
    def compute_index_mean(self):
        self.index =   self.mean_reward/(self.mean_cost) # +  np.sqrt( alpha_1 * np.log(time) / (self.chosen_time) )/self.mean_cost_ # or self.mean_cost 这个影响不大
        return self.index 
    # EABS-TS
    ...
    def compute_index_update_without_term(self):
        self.index = self.mean_reward/(self.mean_cost) # +  np.sqrt( alpha_1 * np.log(time) / (self.chosen_time) )/self.mean_cost_ # or self.mean_cost 这个影响不大
        return self.index
        ...
        for num in range(simulation_Num_):
            ...
            for u in range(U_temp):
                for i in range(N):
                    ...
                    cost_U_N[u][i] = data_5G[ num ][u][i] + policy_SU_set[u][i].channel_info.c_allocation_probing_ACK - 1e-02 + 2.5e-04  # 不考虑信道分配的能耗，但是假设 Gale-Shapley 信道分配耗能 2.5e-04 J
            ...
            if num < 1:
                ...
                for u in range(U):
                    for i in range(N):
                        ...
                        # UCB without energy
                        index_U_N[u][i] = policy_SU_set[u][i].compute_index_update_MAB_without_energy( num )
                ...
                User_preference = []
                for u in range(U):
                    mixer = np.random.random(index_U_N[u].size)
                    ucb_indices = list(np.lexsort((mixer, index_U_N[u]))) 
                    User_preference.append( ucb_indices[::-1] )
                
                Channel_Preference = [] 
                for i in range(N):
                    mixer = np.random.random(index_U_N.shape[0])
                    ucb_indices = list(np.lexsort((mixer, [ index_U_N[u][i] for u in range(U) ]   ))) 
                    Channel_Preference.append( ucb_indices[::-1] )                    
                
                allocation_relationship = GS_matching(User_preference, Channel_Preference)
