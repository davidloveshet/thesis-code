...
说明：
- 由于在仿真中若实时生成5G信道增益，则需要花费极大的时间（有时需要几十个小时）
- 为解决上述问题，基于5G信道增益产生能量消耗样本（见文件 generate_5G_SNR.py）
- 5G 信道增益由文件 Fast_fading_model_with_addtional_modeling_components.py 生成
...
Data_samplings_5G_UMi_Street_canyon_11 = np.load(file = "Data_samplings_5G_UMi_Street_canyon_11.npy")  # 0.21001564692464939
Data_samplings_5G_UMi_Street_canyon_12 = np.load(file = "Data_samplings_5G_UMi_Street_canyon_12.npy")  # 0.21001564692464939
...
Data_samplings_5G_UMi_Street_canyon_11 = Data_samplings_5G_UMi_Street_canyon_11.tolist()
Data_samplings_5G_UMi_Street_canyon_12 = Data_samplings_5G_UMi_Street_canyon_12.tolist()
        ...
        for num in range(simulation_Num_):
            ...
            if num < 1:
                ...
                if np.random.random() < epsilon_:
                    for u in range(U):
                        for i in range(N):                    
                            index_U_N[u][i] = np.random.random()
                else:
                    # index_U_N[u][i] = policy_SU_set[u][i].compute_index_update_greedy()
                    pass
                '''            
                ################################################################################            
