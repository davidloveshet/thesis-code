        ...
        for num in range(simulation_Num_):
            ...
            for u in range(U_temp):
                for i in range(N):
                    ...
                    cost_U_N[u][i] = data_5G[ num ][u][i] + policy_SU_set[u][i].channel_info.c_allocation_probing_ACK - 1e-02  # 仅考虑探测 和 ACK 的能耗
            ...
            if num < 1:
                ...
                # ''' random algorithm, 次要用户随机选取信道接入，冲突的信道将无法成功传输，造成能量浪费 # 
                allocation_relationship_origin = []
                for u in range(U_temp):
                    allocation_relationship_origin.append( [ u, np.random.randint(N) ])  
                ...
                for j in range( len( allocation_relationship ) ):
                    ...
                    channel_access_set.append( allocation_relationship[j][1] ) # 将接入的信道添加到列表中
                # print(channel_access_set)
                # '''                    
                ...
                for j in range( len( allocation_relationship ) ):
                    ...
                    if channel_access_set.count(temp_channel) > 1: # 表示发生冲突的信道 
                        reward_U_N[temp_SU][temp_channel] = copy.deepcopy( np.random.random()/1e06 )
