...
alpha_0 = 1/3  # 对应文中的 alpha = 3 
alpha_1 =  1/30
budget_list = [     3, 5, 10, 20, 25, 30, 35, 40,   50, 60, 70     ] # J 次要用户能量
# budget_list = [   40, 45, 50  ] # J
Monte_Carlo_Num = 30 # 仿真次数
...
transmission_distance = [ 315.1545, 324.23451, 333.535329,  345.2345543,  349.85736952,  353.32453458 ] # m
...
class algo:
    ...
    def compute_index_update(self, time):
        self.index = alpha_0 * self.mean_reward/(self.mean_cost) +  np.sqrt( alpha_1 * np.log(time) / (self.chosen_time) )/mean_cost
        return self.index
    ...
    for monte in range(Monte_Carlo_Num):
        ...
        for num in range(simulation_Num_):
            ...
            if num < 1:
                ...
                # """
                # 添加 Static Bertsekas 模块进行信道分配，该方法得到的是 Benchmark 
                # """
                ''' origin auction 
