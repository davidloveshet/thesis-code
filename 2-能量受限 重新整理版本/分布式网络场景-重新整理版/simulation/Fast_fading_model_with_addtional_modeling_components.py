...
@Date    :   2022/10/13
@Version :   1.1
@parameter:  modify the channel parameter generation and the pathloss parameter
...
#     Scenario = 'UMi_Street_canyon' # RMa, UMa, UMi_Street_canyon, InH_Office   Rural, Urban, Street 
#     d_2D_out = 100
#     f_c = 6 #*1000*1000*1000  
...
# In[0] step1 Set environment, network layout, and antenna array parameters 设置环境、网络布局和天线阵列参数  TODO Need someone to verify the parameters for me 
...
# f_c = 3*1000*1000*1000    #Specify system centre frequency  3Ghz
# B = 20*1000*1000     #bandwidth   20MKHz

# In[0] step2  Assign propagation condition (LOS/NLOS)  确定传播条件
...
# In[0] step3  Calculate pathloss with formulas in Table 7.4.1-1 for each BS-UT link to be modelled. 计算每条链路的路径损失(包含阴影衰落)
...
def Generate_large_scale_parameters(scenario, d_2D, f_c):
    lsp = {}
    if scenario == 'RMa':
...
# In[0] step5  Generate cluster delays  τ_n
...
# In[0] step6 Generate cluster powers P_n
...
# In[0] step7   Generate arrival angles and departure angles for both azimuth and elevation.
C_φ_NLOS_A = {
    '4':0.779,
    '5':0.860,
    '8':1.108,
...
# In[0] step8  Coupling of rays within a cluster for both azimuth and elevation

# In[0] step9  Generate the cross polarization power ratios
...
# In[0] step10   Draw initial random phases
...
# In[0] step11  Generate channel coefficients for each cluster n and each receiver and transmitter element pair u, s.
def  Generate_channel_coefficients(lsp,propagation_condition,f_c,  φ_n_m_AOA,φ_n_m_AOD,θ_n_m_ZOA,θ_n_m_ZOD,  Fei_θθ_n_m,Fei_θφ_n_m,Fei_φθ_n_m,Fei_φφ_n_m,  XPR_n_m,  τ_n,  P_n, d_2D_out ):
    ...
    for i in range(N):
        ...
        for j in range(M):
            if i < 2:
                ...
                for k in range(20):
                    if k < 8 or k == 18 or k == 19:
                        ...
                        temp_result1.append(H_u_s_n_m_NLOS_t*1/20)  # default 3.91 ns  step 5         
                    if k >= 8 and k < 12 or k == 16 or k == 17:
                        temp_result1.append(H_u_s_n_m_NLOS_t*1/20) 
                    if k >= 12 and k < 16:
                        temp_result1.append(H_u_s_n_m_NLOS_t*1/20) 
         ...
         # 有待修正
        if i < 2:
            H_u_s_n_NLOS_t_1_and_2.append(np.linalg.norm(temp_result1))
        else:
            H_u_s_n_NLOS_t_3_N.append( np.linalg.norm(temp_result2))
        
        # print('H_u_s_n_NLOS_t_1_and_2', H_u_s_n_NLOS_t_1_and_2)
        # print('H_u_s_n_NLOS_t_3_N', H_u_s_n_NLOS_t_3_N)
    H_u_s_NLOS_τ_t =  H_u_s_n_NLOS_t_1_and_2 + H_u_s_n_NLOS_t_3_N
        
    if propagation_condition != 'LOS':
        return H_u_s_NLOS_τ_t
    else:    
        d_3D = math.sqrt(math.pow(d_2D_out, 2)+math.pow(abs(h_BS-h_UT), 2))
        H_u_s_1_LOS_t = np.dot( np.dot(np.array([F_rx_u_θ[0],F_rx_u_φ[0]]),np.array([[cmath.exp(complex(0, Fei_θθ_n_m[0])),math.sqrt(1/XPR_n_m[0])*cmath.exp(complex(0, Fei_θφ_n_m[0]))],[math.sqrt(1/XPR_n_m[0])*cmath.exp(complex(0, Fei_φθ_n_m[0])),cmath.exp(complex(0, Fei_φφ_n_m[0]))]])), np.array([[F_tx_s_θ[0]],[F_tx_s_φ[0]]]) )                   * cmath.exp(-complex(0, 1)*2*np.pi*d_3D/Lambda_0) * cmath.exp(complex(0, 1)*2*np.pi*np.dot(r_rx_LOS ,np.transpose(d_rx_u))/Lambda_0) * cmath.exp(complex(0, 1)*2*np.pi*np.dot(r_tx_LOS ,np.transpose(d_tx_s))/Lambda_0) * cmath.exp(complex(0, 1)*2*np.pi*np.dot(r_rx_LOS , np.transpose(v_))/Lambda_0)
        ...
        # H_u_s_1_LOS_t = cmath.exp(complex(0, 1)*2*np.pi*d_3D/Lambda_0) * cmath.exp(complex(0, 1)*2*np.pi*np.dot(r_rx_LOS ,np.transpose(d_rx_u))/Lambda_0) * cmath.exp(complex(0, 1)*2*np.pi*np.dot(r_tx_LOS ,np.transpose(d_tx_s))/Lambda_0) * cmath.exp(complex(0, 1)*2*np.pi*np.dot(r_rx_LOS , np.transpose(v_))/Lambda_0)
        ...
        # print(math.sqrt(K/(K+1)))
        # print(K)
        for i in range(len(H_u_s_NLOS_τ_t)):
            H_u_s_NLOS_τ_t[i] = math.sqrt(math.sqrt(abs(1/(K+1)))) * H_u_s_NLOS_τ_t[i] + 0.0
            
        H_u_s_NLOS_τ_t.append(np.linalg.norm(math.sqrt(abs(K/(K+1))) * H_u_s_1_LOS_t))
        return H_u_s_NLOS_τ_t
...
def main(scenario, d_2D_out, f_c):
...
# if __name__=='__main__': #应该使所有的函数以某个周期时间（越快越好）快速迭代运行
    ...
    PL_LOS , PL_NLOS = Calculate_pathloss(scenario, d_2D_out, f_c,h_BS = 50,h_UT = 1.5,h = 5,W = 20)  # step3 计算每条链路的路径损失(包含阴影衰落)
    # print(PL_LOS , PL_NLOS)
    lsp = Generate_large_scale_parameters(scenario, d_2D_out, f_c)  #step4 生成大尺度参数 
    # print(lsp)
    ...
    H_u_s = Generate_channel_coefficients(lsp,propagation_condition,f_c,φ_n_m_AOA_,φ_n_m_AOD_,θ_n_m_ZOA_,θ_n_m_ZOD_,  Fei_θθ_n_m,Fei_θφ_n_m,Fei_φθ_n_m,Fei_φφ_n_m,  XPR_n_m_,  τ_n_,  P_n_, d_2D_out )
    # print('H_u_s',H_u_s)
    # print('the norm form is', np.linalg.norm(H_u_s) )
    # #step12   Apply pathloss and shadowing for the channel coefficients.

    PL_LOS_ = np.sqrt(10 **(- PL_LOS/10))
    PL_NLOS_ = np.sqrt(10 **(- PL_NLOS/10))
    # print('The LOS path loss is', PL_LOS_)
    # print('The NLOS path loss is', PL_NLOS_)        
    
    # result.append([H_u_s])
    if propagation_condition == 'LOS':
        temp_ = [ H_u_s[i] * PL_NLOS_ for i in range(len(H_u_s)-1) ] #.append( H_u_s[-1] * PL_LOS_ )
        temp_1 = H_u_s[-1] * PL_LOS_
        temp_.append(temp_1)
        # print("The LOS H is", np.linalg.norm(temp_) )
        link_para = np.linalg.norm(temp_)
    else:
        temp_ = [ H_u_s[i] * PL_NLOS_ for i in range(len(H_u_s)) ]
        # link_para = np.linalg.norm(temp_)
        # print("The NLOS H is", np.linalg.norm(temp_) )
        link_para = np.linalg.norm(temp_)
    
    return link_para
    
    # Channel_Noise = -94 # dBm
    # Channel_Noise = 10**(Channel_Noise/10)/(10**3) 
    # Link_Gain = link_para**2/Channel_Noise
    # print("The link gain is", Link_Gain)
