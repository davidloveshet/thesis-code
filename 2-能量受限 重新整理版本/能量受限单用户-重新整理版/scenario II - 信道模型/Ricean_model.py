    ...
    for monte in range(Monte_Carlo_Num):
        ...
        for num in range(simulation_Num_):
            ...
            if num < 1: # 初始化相关信息，没有 reward 和 能耗 
                ...
                for i in range(N):
                    index[i] = policy[i].compute_index_update(reward[i], num) # 次要用户计算每条信道标号
...
# 该处最优能量效率假设次要用户知道信道统计信息，因此根据 \mu_i^r 和 \mu_i^c 比值最大的 M 进行感知接入。设置较大能量（设置为 20 J）和蒙特卡洛次数（设置为25）可得到 benchmark 值
    if M == 1:
        average_reward_set_benchmark = 110.907
    elif M ==2:
        average_reward_set_benchmark = 100.8108
    elif M == 3:
        average_reward_set_benchmark = 90.9354
    elif M == 4:
        average_reward_set_benchmark = 82.4292
    else:
