...
@author: xxx
feature_generation 对应三个不同的 QoS 需求
M 为信道感知数量
...
ALGO = 'Fixed' # 'random', 'proposed', 'greedy', 'Fixed' ; 选取不同的算法得到对应的仿真结果
...
for m_c in range(m_c_number):
    ...
    for i in range(len(policy_1)):
        policy_1[i].re_init() # 初始化
        policy_2[i].re_init() # 初始化        
        policy_3[i].re_init() # 初始化        
        ...
        if t < T_init: # initialization, each channel has been sensed
            ...
            # if t < T_thres_1:    
                ...
                for element in chosen_arm_1:
                    if channel_state[element] == 1: # 如果信道被感知为空闲
                ...
                for element in chosen_arm_1: #########
                    ...
                    if channel_state[element] == 1: # 如果信道被感知为空闲，则接入
                        Throughput_mc_1[m_c][t] += 1 # * transmission_rate * transmission_duration # 因为空闲，所以传输成功
            ...
            # elif T_thres_1 < t < T_thres_2:  
                ...
                for element in chosen_arm_2: #########
                    ...
                    if channel_state[element] == 1: # 如果信道被感知为空闲，则接入
                        Throughput_mc_2[m_c][t] += 1 # * transmission_rate * transmission_duration # 因为空闲，所以传输成功
            ...
            # 更新 s = 3 时的                
                ...
                for element in chosen_arm_3:  
                    ...
                    if channel_state[element] == 1: # 如果信道被感知为空闲，则接入
                        Throughput_mc_3[m_c][t] += 1 # * transmission_rate * transmission_duration # 因为空闲，所以传输成功
