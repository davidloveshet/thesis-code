    ...
    for t in range(T_simulation):
        ...
        if t < T_init: # initialization, each channel has been sensed
            for i, theta_index in enumerate(arm_theta_set):
                ...
                # 因为各用户业务类型相同，因此采用 reward_SU_1更新 
...
for i,element in enumerate(regret_x_label):
    ...
    # cumulative_collision_y_label[i] = cumulative_collision_y_label[i]/(m_c_number*element)
...
'''
注意的是每次跑出的冲突数量不同，但都是最初始线性增长后期达到不变。
因此达到后期便正交。所以选取了一次的结果作为记录。也可以取多次仿真取统计平均。
'''
