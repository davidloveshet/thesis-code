...
This file is used to simulate the heterogeneous network with M=K=1, U=3
"""
...
T_simulation = 16000 # the overall simulation time
m_c_number = 30
ALGO = 'proposed' # 'random', 'fixed', 'proposed'
...
# arm_theta_set_SU_1 = [
# np.array([0.65, 0.20, 0.25, 0.63]),
# np.array([0.27, 0.20, 0.63, 0.61]),
# np.array([0.32, 0.25, 0.53, 0.10]),
# np.array([0.37, 0.25, 0.22, 0.66]),
# np.array([0.20, 0.30, 0.13, 0.84]),
# np.array([0.43, 0.30, 0.83, 0.31]),
# np.array([0.48, 0.35, 0.65, 0.16]),
# np.array([0.53, 0.35, 0.52, 0.14]),
# np.array([0.59, 0.40, 0.49, 0.35])]

# arm_theta_set_SU_1 = [ arm_theta_set_SU_1[i].reshape(4,1) for i in range(9) ]
...
for m_c in range(m_c_number):
    ...
    for t in range(T_simulation):
        ...
        if t < T_init: # initialization, each channel has been sensed
            ...
            Reward_Matrix_estimated = [ ucb_s_SU_1, ucb_s_SU_2, ucb_s_SU_3 ]
            # if M_U == 2:
            ...
            if ALGO == 'fixed':
                optimal_result = copy.deepcopy( [  [0,6], [1,7], [2,8]  ] )
            ...
            for j in range(len(optimal_result)):
                if optimal_result[j][0] == 0: # SU 1, accesses the [j][1] channel
                    ...
                    Reward_all_simulation_mc_SU_1[m_c][t] = reward_SU_1[channel_access] # np.dot(feature_generation_1.T, theta_generation_1[channel_access])
                ...
                elif optimal_result[j][0] == 1: # SU 2, accesses the [j][1] channel
                    channel_access = optimal_result[j][1]
                    ...
                    Reward_all_simulation_mc_SU_2[m_c][t] = reward_SU_2[channel_access] # np.dot(feature_generation_2.T, theta_generation_2[channel_access])
                    ...
                    Reward_all_simulation_mc_SU_3[m_c][t] = reward_SU_3[channel_access] # np.dot(feature_generation_3.T, theta_generation_3[channel_access])
...
plt.title('The average throughput')
plt.show()



# we record the 'cumulative_regret_y_label', 'average_throughput_1', and 'average_Price_1' in file 'plot_algorithm_price_QoS_Central.py'
