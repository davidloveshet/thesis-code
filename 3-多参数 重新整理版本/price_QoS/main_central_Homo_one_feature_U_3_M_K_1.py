...
Homogeneous centralized network  
"""
...
ALGO = 'proposed' # 'random', 'fixed', 'proposed'. select various algorithms, we can obtain the corresponding result
...
# M = 6   # the sensed channel number
# K = 6   # the accessed channel number
...
for m_c in range(m_c_number):
    ...
    for t in range(T_simulation):
        ...
        if t < T_init: # initialization, each channel has been sensed
            ...
            Reward_Matrix_estimated = [ ucb_s_SU_1, ucb_s_SU_2, ucb_s_SU_3 ] # the upper confidence bound of the estimated utility
...
plt.plot(regret_x_label, cumulative_regret_y_label, label='regret')
...
plt.plot(regret_x_label, average_Price_1,'-^',label = 'average price')
plt.plot(regret_x_label, average_Price_2,'-^')
plt.plot(regret_x_label, average_Price_3,'-^')
...
plt.plot(regret_x_label, average_throughput_1,'-^', label = 'average throughput')
plt.plot(regret_x_label, average_throughput_2,'-^')
plt.plot(regret_x_label, average_throughput_3,'-^')
